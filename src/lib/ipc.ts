...
export async function authLogin(email: string, password: string): Promise<AuthSession> {
  return invoke("auth_login", { email, password });
}

export async function authRegister(email: string, password: string): Promise<AuthSession> {
  return invoke("auth_register", { email, password });
}
...
export async function authRefresh(): Promise<AuthSession> {
  return invoke("auth_refresh");
}

export async function authGetSession(): Promise<AuthSession> {
  return invoke("auth_get_session");
}

export async function authGetSubscription(): Promise<AuthSession["subscription"]> {
  return invoke("auth_get_subscription");
}
